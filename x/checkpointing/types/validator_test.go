...
import (
	...
	"github.com/hetu-project/hetu/v1/testutil/datagen"
...
func TestValidatorSet_FindValidatorWithIndex(t *testing.T) {
	...
	for i := 0; i < len(valSet1); i++ {
		val, index, err := valSet1.FindValidatorWithIndex(common.Address(valSet1[i].Addr))
		require.NoError(t, err)
		require.Equal(t, val, &valSet1[i])
		require.Equal(t, index, i)
	}
	val, index, err := valSet1.FindValidatorWithIndex(common.Address(valSet2[0].Addr))
	require.Error(t, err)
	require.Nil(t, val)
