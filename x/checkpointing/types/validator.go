...
func (v *Validator) GetValAddress() common.Address {
	return common.Address(v.Addr)
}
...
func (vs ValidatorSet) FindValidatorWithIndex(valAddr common.Address) (*Validator, int, error) {
	index := vs.binarySearch(valAddr.Bytes())
	if index == -1 {
...
func (vs ValidatorSet) binarySearch(targetAddr []byte) int {
	var lo = 0
	var hi = len(vs) - 1
